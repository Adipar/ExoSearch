...
class ExoplanetHabitabilityUI:
    ...
    def display_top_candidates(self):
        """Display the top Earth-like exoplanet candidates with their names"""
        st.markdown('<h2 class="sub-header">🏆 Most Earth-Like Exoplanets</h2>', unsafe_allow_html=True)
        ...
        # Check if we have planet names in the dataset
        has_planet_names = 'pl_name' in filtered_data.columns
        ...
        if has_planet_names:
            ...
            # If no planet names, create generic names
            top_candidates = filtered_data[
                (filtered_data['target_class'] == 'CONFIRMED')
            ].nlargest(10, 'earth_similarity_score')
            top_candidates['pl_name'] = [f"Exoplanet {i + 1}" for i in range(len(top_candidates))]
        ...
        for idx, (_, candidate) in enumerate(top_candidates.iterrows(), 1):
            ...
            planet_name = candidate.get('pl_name', f"Candidate {idx}")
            ...
            with col1:
                ...
                <div class='metric-card {card_class}'>
                    <h3>🌍 {planet_name}</h3>
                    <p><strong>Earth Similarity:</strong> {similarity:.1f}% ({category})</p>
                </div>
                """, unsafe_allow_html=True)
        ...
        with st.expander("View Detailed Candidate Data"):
            display_columns = ['pl_name'] if 'pl_name' in top_candidates.columns else []
    ...
    def create_prediction_interface(self):
        """Create interface for users to input planet parameters and predict habitability"""
        st.markdown('<h2 class="sub-header">🔮 Predict Habitability for Custom Planets</h2>', unsafe_allow_html=True)

        with st.form("planet_prediction_form"):
            st.subheader("Enter Planetary Parameters")

            col1, col2 = st.columns(2)

            with col1:
                planetary_radius = st.number_input(
                    "Planetary Radius (Earth radii)",
                    min_value=0.1,
                    max_value=20.0,
                    value=1.0,
                    step=0.1,
                    help="Radius relative to Earth (Earth = 1.0)"
                )
                ...
                equilibrium_temp = st.number_input(
                    ...
                    value=255,
                    step=10,
                    help="Planet's calculated equilibrium temperature"
                )
            ...
            with col2:
                orbital_period = st.number_input(
                    ...
                    value=365.25,
                    step=10.0,
                    help="Time to complete one orbit around star"
                )
                ...
                stellar_radius = st.number_input(
                    ...
                    value=1.0,
                    step=0.1,
                    help="Radius of host star relative to Sun"
                )

            # Submit button
            submitted = st.form_submit_button("Predict Habitability")

            if submitted:
                self.predict_habitability(
    ...
    def predict_habitability(self, **planet_params):
        """Predict habitability probability using the trained model"""
        try:
            # Calculate Earth similarity
            similarity_score, similarities = self.similarity_calculator.calculate_similarity_score(planet_params)

            # Prepare data for model prediction
            planet_data = {
                'orbital_period': planet_params.get('orbital_period', 365.25),
                'planetary_radius': planet_params.get('planetary_radius', 1.0),
                'insolation_flux': planet_params.get('insolation_flux', 1.0),
                'equilibrium_temp': planet_params.get('equilibrium_temp', 255),
                'stellar_temp': planet_params.get('stellar_temp', 5778),
                'stellar_radius': planet_params.get('stellar_radius', 1.0),
                'stellar_mass': 1.0,  # Default value
                'distance': 100,  # Default value
                'earth_similarity_score': similarity_score
            }

            # Create DataFrame for prediction
            prediction_df = pd.DataFrame([planet_data])

            # Select features used in training (adjust based on your model)
            feature_columns = [
                'orbital_period', 'planetary_radius', 'insolation_flux',
                'equilibrium_temp', 'stellar_temp', 'stellar_radius',
                'stellar_mass', 'distance', 'earth_similarity_score'
            ]

            # Ensure we have all required columns
            for col in feature_columns:
                if col not in prediction_df.columns:
                    prediction_df[col] = 0  # Default value for missing columns

            prediction_features = prediction_df[feature_columns]

            # Handle missing values
            prediction_features = self.imputer.transform(prediction_features)

            # Scale features
            prediction_features = self.scaler.transform(prediction_features)
            ...
            with col1:
                ...
                st.metric(
                    ...
                    delta=f"{similarity_score - 50:.1f}% from habitable threshold" if similarity_score >= 50 else None,
                    delta_color="normal" if similarity_score >= 50 else "inverse"
                )

            with col2:
                # Habitability Probability
                habitability_prob = prediction_proba[0][1] * 100  # Assuming class 1 is habitable
                st.metric(
                    "Habitability Probability",
                    f"{habitability_prob:.1f}%",
                    delta=f"{habitability_prob - 50:.1f}%" if habitability_prob >= 50 else None,
                    delta_color="normal" if habitability_prob >= 50 else "inverse"
                )

            with col3:
                # Predicted Class
                status_color = "🟢" if predicted_class == "CONFIRMED" else "🟡" if predicted_class == "CANDIDATE" else "🔴"
                st.metric("Predicted Status", f"{status_color} {predicted_class}")

            # Detailed breakdown
            st.subheader("Detailed Analysis")

            # Feature comparison with Earth
            st.write("**Feature Comparison with Earth:**")
            comparison_data = {
                'Parameter': ['Planetary Radius', 'Insolation Flux', 'Equilibrium Temperature',
                              'Orbital Period', 'Stellar Temperature', 'Stellar Radius'],
                'Your Planet': [
                    f"{planet_params.get('planetary_radius', 1.0):.2f} Earth radii",
                    f"{planet_params.get('insolation_flux', 1.0):.2f} Earth fluxes",
                    f"{planet_params.get('equilibrium_temp', 255):.0f} K",
                    f"{planet_params.get('orbital_period', 365.25):.1f} days",
                    f"{planet_params.get('stellar_temp', 5778):.0f} K",
                    f"{planet_params.get('stellar_radius', 1.0):.2f} Solar radii"
                ],
                'Earth': [
                    "1.00 Earth radii",
                    "1.00 Earth fluxes",
                ...
                'Similarity': [f"{sim * 100:.1f}%" for sim in similarities.values()]
            }
            ...
            if similarity_score >= 70:
                st.success("""
                **High Earth Similarity!** This planet shows strong potential for habitability.
                - Planetary characteristics are very similar to Earth
                - Likely located in the habitable zone
                - Good candidate for further study
                """)
            elif similarity_score >= 50:
                st.warning("""
                **Moderate Earth Similarity.** This planet may have some habitable conditions.
                - Some parameters differ significantly from Earth
                - Could support extreme forms of life
                - Worth further investigation
                """)
            else:
                st.error("""
                **Low Earth Similarity.** This planet is unlikely to be habitable by Earth-like life.
                - Significant differences from Earth's conditions
                - Harsh environment for known life forms
                - Low priority for habitability studies
                """)

        except Exception as e:
            st.error(f"Error making prediction: {e}")
            st.info("Please check that all required parameters are provided and within reasonable ranges.")
    ...
    def run(self):
        ...
        else:  # Predict Habitability section
